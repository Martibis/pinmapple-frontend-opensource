<?php
    ...
    if(!empty($_GET['author'])){
        $author = "AND username = '" . mysqli_real_escape_string($conn, $_GET['author']) . "' ";
    }else{
        $author = '';
    }
    if(!empty($_GET['tag'])){
    ...
    $sql    = "SELECT postLink, longitude, lattitude, postDate FROM markerinfo WHERE lattitude != 0 AND longitude != 0  $post $author $tag $editorsChoice $dateRange";
